# 🔄 Agent Sync - 大模型上下文同步工具

## 📖 简介

Agent Sync 是一个将 Godot 项目代码、配置和规则文档合并成单一 XML 文件的工具，专为大模型（Gemini/Claude）优化。
...
### 手动运行
```bash
python AISpace/.agent/CloudSync_Workflow/agent_sync_to_drive.py
```

### 自动运行（Hook）
脚本会在以下情况自动触发：
- 保存 `.md`, `.json`, `.txt` 文件（AISpace）
- 保存 `.cs`, `.gd`, `.tscn` 文件（TetrisBackpack）
...
### 路径配置
```python
SOURCE_PROJECT_DIR = r"C:\Godot\TetrisBackpack"           # Godot 项目目录
AGENT_WORKSPACE_DIR = r"C:\Godot\AISpace\.agent" # Agent 规则目录
DRIVE_SYNC_PATH = r"C:\Users\26070\My Drive\Agent_Godot_Brain" # Google Drive
LOCAL_BACKUP_PATH = r"D:\Agent_Godot_Brain_Backup"      # 本地备份
```
...
<system_context>
  ...
  <section_1_important_rules>
    <file path=".agent/ProjectRules.md" priority="CRITICAL">
<![CDATA[
... 核心规则内容 ...
]]>
    </file>
  ...
  <section_2_project_code>
    <file path="TetrisBackpack/addons/A1TetrisBackpack/..." priority="NORMAL">
<![CDATA[
... 源码内容 ...
]]>
    </file>
...
#### .agent/steering\Always\MainRules.md
```diff
@@ -3,11 +3,11 @@
...
### 问题：变更检测不工作
**检查**:
1. `D:\Agent_Godot_Brain_Backup\AI_Context_Manifest.json` 是否存在
2. 文件权限是否正确
3. 查看脚本输出的错误信息

### 问题：Hook 不触发
**检查**:
1. Hook 配置文件是否存在（`.agent/hooks/`）
2. 查看 Hook 日志（`.agent/CloudSync_Workflow/logs/`）
3. 使用 Agent 命令面板检查 Hook 状态
...
Agent Sync 是一个基于大模型认知原理设计的上下文同步工具，采用 XML 结构和三明治布局，智能过滤和变更检测，自动同步到 Google Drive，为 AI 提供最优质的项目上下文。
