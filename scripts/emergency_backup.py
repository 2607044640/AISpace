...
def emergency_backup():
    ...
    # 定义关键文件
    critical_files = [
        "AISpace/.kiro/docLastConversationState.md",
        "AISpace/.kiro/ProjectRules.md",
        "AISpace/.kiro/Scratchpad/BackpackTest_BugFix_Guide.md",
        "AISpace/.kiro/Scratchpad/GuiInput_Bug_Fix_Summary.md",
    ]
    
    # 创建备份目录
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_dir = Path(f"AISpace/.kiro/chat_backups/{timestamp}_emergency")
    ...
    for file_path in critical_files:
        ...
        if src.exists():
            # 保持相对路径结构
            rel_path = src.relative_to("AISpace/.kiro")
    ...
    # 备份整个 Scratchpad 目录
    scratchpad_src = Path("AISpace/.kiro/Scratchpad")
